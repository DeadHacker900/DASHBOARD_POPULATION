...
export async function POST(request: Request) {
  try {
    const { query, type = "demographic", depth = "detailed" } = await request.json()

    // Create a prompt based on the request
    const prompt = createPrompt(query, type, depth)

    // If OpenAI API key is available, use the AI SDK
      ...
      try {
        ...
        try {
          ...
          return NextResponse.json({
            ...
            insight: {
              id: `insight-${Date.now()}`,
              title: generateTitle(query, type),
              content: text,
        ...
        return NextResponse.json({
          success: true,
          insight: generateMockInsight(query, type, depth),
        })
      }
    } else {
    ...
    return NextResponse.json({ success: false, message: "Failed to generate AI insight" }, { status: 500 })
  }
}

// Create a prompt for the AI model
function createPrompt(query: string, type: string, depth: string): string {
...
function generateMockInsight(query: string, type: string, depth: string): any {
  const now = new Date()
  ...
  return {
    ...
    model: "mixture-of-experts-simulation",
    ...insight,
  }
}
