...
import { MLArchitectureSimplified } from "./ml-architecture-simplified"

export function MLArchitectureMermaid() {
  const [diagramLoaded, setDiagramLoaded] = useState(false)
  const [error, setError] = useState(false)

  useEffect(() => {
    // Try to load mermaid diagram
  ...
  if (error) {
    return (
      <div className="w-full h-full">
        <div className="text-amber-400 mb-4 text-sm">
          Interactive diagram could not be loaded. Showing simplified version instead.
        </div>
        <MLArchitectureSimplified />
      </div>
    )
  }
  ...
  return (
    <div className="w-full h-full">
      <div id="mermaid-diagram" className="w-full h-full"></div>
    </div>
  )
}
